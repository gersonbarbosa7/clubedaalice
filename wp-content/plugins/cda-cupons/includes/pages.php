...
function cda_cupons_settings_page() {
    ...
    if ($cupons || $lista_restricao):
        ...
        if ($excluircupom){
            ...
            $existe = $wpdb->get_row("SELECT * FROM ".$wpdb->prefix . 'cda_codigos'." WHERE id = '".$excluircupom."'");

            // $wpdb->query("DELETE FROM ".$wpdb->prefix . 'cda_cupons_uso'." WHERE cupom_id = '".$excluircupom."'");
            if($existe->email) {
                $wpdb->query("DELETE FROM ".$wpdb->prefix . 'cda_cupons_uso'." WHERE cupom_id = '".$cupons."' AND email = '".$existe->email."'");
            }
        ...
        if ($excluirpart){
            ...
            $wpdb->query("DELETE FROM ".$wpdb->prefix . 'cda_cupons_uso'." WHERE cupom_id = '".$existe->id."' LIMIT 1");
            $wpdb->query("UPDATE ".$wpdb->prefix . 'cda_codigos'." SET usado_em = NULL, email = NULL WHERE codigo = '".$excluirpart."'");
